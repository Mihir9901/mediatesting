...
const getActor = (req) => {
  ...
  // Manager comes from Manager model
  if (req.manager) {
    return {
      id: req.manager.id,
      role: 'manager',
...
exports.createTeam = async (req, res) => {
  try {
    ...
    const exists = await Team.findOne({ name: trimmed });
    if (exists) {
      return res.status(400).json({ message: 'A team with this name already exists' });
    }
...
// @route   POST /api/teams/:teamId/head
exports.setTeamHead = async (req, res) => {
  try {
    const { name, email, password } = req.body;
    ...
    res.status(201).json({
      message: 'Team head account created successfully',
      teamHead: {
        id: head._id,
        name: head.name,
        email: head.email,
...
exports.removeMember = async (req, res) => {
  try {
    ...
    team.memberEmployeeIds = (team.memberEmployeeIds || []).filter(
      (id) => id !== employeeId
    );
