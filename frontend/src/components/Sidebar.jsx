...
// import { LayoutDashboard, Building2, Users, UserRound, FileSpreadsheet, CalendarCheck, ClipboardList, UserCog, UserCheck, CalendarClock, Shield, Key } from 'lucide-react';
...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  const links =
    user.role === 'Admin'
      ? [
          ...
          { path: '/admin/managers', label: 'Managers', icon: UserRound },
          { path: '/admin/team-heads', label: 'Team Heads', icon: UserCheck },
          { path: '/admin/create-team-head', label: 'Create Team Head', icon: UserPlus },
          { path: '/admin/upload', label: 'Upload Interns', icon: Users },
          { path: '/admin/interns', label: 'Intern Details', icon: UserCog },
          { path: '/admin/edit-attendance', label: 'Edit Attendance', icon: CalendarClock },
          { path: '/admin/report', label: 'Reports', icon: FileSpreadsheet },
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:sticky top-0 left-0 z-[80] h-screen w-[290px] shrink-0 text-white transition-transform duration-300 ${
          isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        <div className="relative h-full bg-gradient-to-b from-slate-950 via-slate-900 to-indigo-950 border-r border-white/10 shadow-[24px_0_80px_-36px_rgba(15,23,42,0.9)] overflow-hidden">
          <div
          ...
          <div className="relative flex flex-col h-full">
            ...
            <div className="px-5 pt-5 pb-4">
              <div className="rounded-3xl border border-white/10 bg-white/[0.04] backdrop-blur-xl px-4 py-4 shadow-xl shadow-black/20">
                <div className="flex items-center justify-center">
            ...
            {/* Nav */}
            <nav className="sidebar-scroll flex-1 px-3 pb-5 overflow-y-auto space-y-1.5">
              {links.map((link) => {
                const Icon = link.icon;
                const isActive = location.pathname === link.path;
                ...
                return (
                  <Link
                    ...
                    to={link.path}
                    onClick={() => window.innerWidth < 1024 && toggleSidebar()}
                    className={`group relative flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition-all ${
                      isActive
                        ? 'bg-white/10 text-white ring-1 ring-white/10 shadow-[0_12px_30px_-18px_rgba(99,102,241,0.9)]'
