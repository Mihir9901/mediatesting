...
const ManagerKHIAccounts = () => {
  ...
  const handleUserToggle = (userId, isEditing = false) => {
    ...
    if (currentIds.includes(userId)) {
      setTarget({
        ...target,
        allowedUsers: current.filter(u => (typeof u === 'object' ? u._id : u) !== userId)
      });
    } else {
      setTarget({
        ...target,
        allowedUsers: [...current, userId]
      });
    }
  };
        ...
        <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 p-6 shadow-2xl sm:p-8">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="text-3xl font-black tracking-tight text-white md:text-4xl">G-Accounts</h1>
              <p className="mt-2 max-w-2xl text-sm font-medium text-slate-300 md:text-base">
                Share, manage, and secure platform credentials across your teams from one controlled workspace.
              </p>
            </div>

            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur-xl">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200">Shared</p>
                <p className="mt-1 text-xl font-black text-white">{accounts.length}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur-xl">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200">Admin</p>
                <p className="mt-1 text-xl font-black text-white">{adminShared.length}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 backdrop-blur-xl col-span-2 sm:col-span-1">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200">Team Heads</p>
                <p className="mt-1 text-xl font-black text-white">{teamHeads.length}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <section className="overflow-visible rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-6 py-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-900">Share New Credentials</h2>
                <p className="text-sm text-slate-500">Create and assign secure credentials to your team heads</p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleCreateSubmit} className="p-6 space-y-6">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
              <div className="space-y-2">
                <label className="px-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">Platform Name</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="e.g. Google Cloud, Meta API"
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-12 pr-4 text-sm font-medium outline-none transition focus:border-indigo-400 focus:bg-white"
                  />
                </div>
              </div>

              <div className="space-y-2">
                <label className="px-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">Username / ID</label>
                <div className="relative">
              ...
              <div className="space-y-2">
                <label className="px-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">Password</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-12 pr-12 text-sm font-medium outline-none transition focus:border-indigo-400 focus:bg-white"
                  />
                  <button
                    type="button"
              ...
              <div className="space-y-2 md:col-span-2 xl:col-span-3">
                <label className="px-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">API Details / Extra Info</label>
                <textarea
                  value={formData.apiDetails}
                  onChange={(e) => setFormData({ ...formData, apiDetails: e.target.value })}
                  placeholder="Paste API keys, secret tokens, or usage instructions here..."
                  className="min-h-[110px] w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none transition focus:border-indigo-400 focus:bg-white"
                />
              </div>

              <div className="relative space-y-2 md:col-span-2 xl:col-span-3" ref={userDropdownRef}>
                <label className="px-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">Share with Team Heads</label>

                <div
                  onClick={() => {
                    if (!showUserDropdown) fetchTeamHeads();
                    setShowUserDropdown(!showUserDropdown);
                  }}
                  className="flex min-h-[54px] w-full cursor-pointer flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2"
                >
                  {formData.allowedUsers.length === 0 ? (
                    <span className="text-sm text-slate-400">Select Team Heads...</span>
                  ) : (
                    formData.allowedUsers.map((u) => {
                      const uid = typeof u === 'object' ? u._id : u;
                      return (
                        <span
                          key={uid}
                          className="inline-flex items-center gap-1 rounded-xl bg-slate-900 px-2 py-1 text-[10px] font-bold text-white"
                        >
                          {teamHeads.find((th) => th._id === uid)?.name || 'User'}
                          <X
                            className="h-3 w-3"
                ...
                {showUserDropdown && (
                  <div className="absolute z-50 mt-2 max-h-80 w-full overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl">
                    {teamHeads.length === 0 ? (
                      <div className="p-6 text-center">
                      ...
                      <>
                        <div className="mb-1 rounded-xl bg-slate-50 px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                          Your Team Heads ({teamHeads.length})
                        </div>
                        {teamHeads.map((th) => {
                          const isSelected = formData.allowedUsers.some((u) => (typeof u === 'object' ? u._id : u) === th._id);
                          return (
                            <div
                              key={th._id}
                              onClick={() => handleUserToggle(th._id)}
                              className="flex cursor-pointer items-center gap-3 rounded-xl p-3 transition hover:bg-indigo-50"
                            >
                              <div className={`h-4 w-4 rounded border ${isSelected ? 'border-indigo-600 bg-indigo-600' : 'border-slate-300'}`}>
                                {isSelected && <Check className="h-3 w-3 text-white" />}
                              </div>
                              <div className="min-w-0 flex-1">
                                <div className="truncate text-sm font-bold text-slate-700">{th.name}</div>
                                <div className="truncate text-[10px] text-slate-400">{th.email}</div>
                              </div>
                            </div>
                          );
                        })}
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-3 rounded-2xl bg-slate-900 px-5 py-4 text-sm font-bold text-white transition hover:bg-slate-800 disabled:opacity-50"
            >
