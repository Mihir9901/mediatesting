...
const EmployeeList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (user?.role === 'TeamHead') {
          const reportRes = await api.get(`/attendance/monthly-report?month=${month}&year=${year}`);
          const rows = reportRes.data.report || [];
          setReportData(rows);
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm font-medium text-slate-300 md:text-base">
                Review intern attendance, task completion, and monthly work summaries.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                onClick={handleRefresh}
                className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-5 py-3 text-sm font-bold text-white backdrop-blur-xl transition hover:bg-white/15"
              >
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-black text-slate-900">{filteredData.length}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            {loading ? (
              ...
              <table className="w-full">
                <thead className="border-b border-slate-200 bg-slate-50">
                  <tr>
                    {[
                      ...
                      <th
                        key={head}
                        className="px-4 py-4 text-left text-[11px] font-black uppercase tracking-[0.2em] text-slate-500"
                      >
                        {head}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-slate-200">
                  {filteredData.map((emp) => (
                    <tr key={emp.uniqueId} className="transition hover:bg-slate-50">
                      <td className="px-4 py-4 text-sm font-medium text-slate-600">{emp.uniqueId}</td>
                  ...
                  {filteredData.length === 0 && (
                    <tr>
                      <td colSpan="10" className="py-14 text-center text-sm font-medium text-slate-500">
                        No records found for this month.
                      </td>
                    </tr>
                  )}
