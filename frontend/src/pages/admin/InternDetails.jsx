  ...
  return (
    <div className="bg-[#f7f8fc] px-4 md:px-6 py-4">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-indigo-100">
                Search, update, and manage all intern records from one place.
              </p>
            </div>

            <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-white">
              <p className="text-xs font-semibold text-indigo-100">Total Interns</p>
              <p className="text-2xl font-black">{interns.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white shadow-sm p-4 md:p-5">
          <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="inline-flex items-center gap-2 rounded-2xl bg-slate-50 border border-slate-200 px-3 py-3">
              ...
              <select
                value={tenureFilter}
                onChange={(e) => setTenureFilter(e.target.value)}
                className="px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-semibold text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500"
              >
                <option value="All">All</option>
                <option value="Active">Active</option>
                <option value="Completed">Completed</option>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900">Intern Records</h2>
                <p className="text-sm text-slate-500">All employee records and tenure details</p>
              </div>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 text-[11px] uppercase tracking-widest text-slate-400">
                  ...
                  <th className="px-6 py-4 text-left">Unique ID</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredInterns.length === 0 ? (
                  ...
                  filteredInterns.map((intern) => (
                    <tr key={intern._id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-5 text-sm text-slate-700">{intern.contactNo || '-'}</td>
                      ...
                      <td className="px-6 py-5">
                        <span className="font-mono text-sm text-slate-600">{intern.employee_id || '-'}</span>
                      </td>

                      <td className="px-6 py-5 text-sm text-slate-700">{formatDate(intern.appliedDate)}</td>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl overflow-hidden">
              ...
              <div className="px-6 pb-6 flex flex-col sm:flex-row gap-3 justify-end">
                <button
                  onClick={closeEdit}
                  className="px-5 py-3 bg-slate-100 text-slate-700 rounded-2xl hover:bg-slate-200 font-semibold text-sm"
                  disabled={saving}
                >
                  Cancel
                </button>
                <button
                  onClick={saveEdit}
                  className="px-5 py-3 bg-indigo-600 text-white rounded-2xl hover:bg-indigo-700 font-semibold text-sm inline-flex items-center justify-center gap-2 disabled:opacity-60"
                  disabled={saving}
                >
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                  Save changes
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-[2rem] shadow-2xl p-6 w-full max-w-md">
              <div className="text-center">
                <div className="mx-auto flex items-center justify-center h-14 w-14 rounded-full bg-red-100 mb-4">
                  <Trash2 className="h-7 w-7 text-red-600" />
                </div>
                ...
                <div className="flex flex-col sm:flex-row justify-center gap-3">
                  <button
                    onClick={cancelDelete}
                    className="px-5 py-3 bg-slate-100 text-slate-700 rounded-2xl hover:bg-slate-200 font-medium text-sm"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={confirmDelete}
                    className="px-5 py-3 bg-red-600 text-white rounded-2xl hover:bg-red-700 font-medium text-sm"
                  >
                    Delete
                  </button>
                </div>
