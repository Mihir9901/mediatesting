...
const AttendanceReport = () => {
  ...
  useEffect(() => {
    if (year) fetchAttendanceData();
  }, [year, month, date, reportType, selectedDepartment, fetchAttendanceData]);

  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden && year) fetchAttendanceData();
    };
  ...
  const handleExport = async (e) => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `Attendance_Report_${selectedDepartment || 'All'}_${year}.xlsx`
      );
  ...
  return (
    <div className="bg-[#f7f8fc] px-4 md:px-6 py-4">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-indigo-100">
                Analyze attendance metrics and export structured Excel reports.
              </p>
            </div>

            <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-white">
              <p className="text-xs font-semibold text-indigo-100">Report Scope</p>
              <p className="text-xl font-black">
                {reportType === 'all' ? 'All Departments' : 'Single Department'}
              </p>
            </div>
          </div>
        </div>
        ...
        {summary && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <SummaryCard icon={Users} label="Total Interns" value={summary.totalEmployees} tone="indigo" />
            <SummaryCard
              icon={CheckCircle}
              label="Present / Completed"
              value={summary.totalCompleted + summary.totalPresent}
              tone="green"
            />
            <SummaryCard icon={XCircle} label="Absent" value={summary.totalAbsent} tone="red" />
            <SummaryCard icon={Clock} label="Total Records" value={summary.totalRecords} tone="amber" />
          </div>
        )}
          ...
          <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-900">Attendance Data</h2>
                  <p className="text-sm text-slate-500">Live attendance breakdown by employee</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={handleRefresh}
                  disabled={dataLoading}
                  className="p-3 rounded-2xl border border-slate-200 text-slate-500 hover:bg-slate-50"
                >
                  <RefreshCw className={`w-4 h-4 ${dataLoading ? 'animate-spin' : ''}`} />
                </button>
              </div>
            ...
            {dataLoading ? (
              <div className="py-14 text-center">
                <div className="inline-flex items-center gap-3 px-6 py-3 bg-indigo-50 rounded-2xl border border-indigo-100">
                  ...
                  <span className="text-sm font-semibold text-indigo-700">Loading Data...</span>
                </div>
              </div>
            ) : reportData.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="bg-slate-50 text-[11px] uppercase tracking-widest text-slate-400">
                      ...
                      <th className="py-4 px-6 text-center">Working Days</th>
                      ...
                      <th className="py-4 px-6 text-center">Incomplete</th>
                    </tr>
                  </thead>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {reportData.map((emp) => (
                      <tr key={emp._id} className="hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="py-4 px-6">
                          <span className="inline-flex items-center text-xs font-semibold text-indigo-700 bg-indigo-50 px-3 py-1 rounded-xl border border-indigo-100">
                            {emp.department}
                          </span>
                        </td>
                        ...
                        <td className="py-4 px-6 text-center">
                          <span className="inline-flex px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-700 text-xs font-bold">
                            {emp.tasksCompleted}
                          </span>
                        </td>
                        ...
                        <td className="py-4 px-6 text-center">
                          <span className="inline-flex px-2.5 py-1 rounded-lg bg-red-100 text-red-700 text-xs font-bold">
                            {emp.absent}
                          </span>
                        </td>
              ...
              <div className="py-12 text-center">
                <div className="inline-flex items-center gap-3 px-6 py-3 bg-slate-50 rounded-2xl border border-slate-200">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Choose scope and export filters</p>
              </div>
            </div>
          ...
          <form onSubmit={handleExport} className="p-6 md:p-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <ToggleCard
                active={reportType === 'all'}
                onClick={() => setReportType('all')}
                icon={LayoutGrid}
                title="All Departments"
                subtitle="Bulk organizational data"
              />
              <ToggleCard
                active={reportType === 'single'}
                onClick={() => setReportType('single')}
                icon={Building2}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <InputWrap label="Specific Date (Optional)" icon={Calendar}>
                <input
                  type="date"
                  value={date}
                  onChange={(e) => setDate(e.target.value)}
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none text-slate-700"
                />
              </InputWrap>

              <InputWrap label="Month" icon={CalendarDays}>
                <select
                  value={month}
                  onChange={(e) => setMonth(e.target.value)}
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none appearance-none text-slate-700"
                >
                  <option value="">Select Month</option>
                  {[
                    'January',
              ...
              <InputWrap label="Year" icon={CalendarDays}>
                <select
                  value={year}
                  onChange={(e) => setYear(e.target.value)}
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none appearance-none text-slate-700"
              ...
              >
                <select
                  ...
                  onChange={(e) => setSelectedDepartment(e.target.value)}
                  disabled={reportType !== 'single'}
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none appearance-none text-slate-700 disabled:opacity-60"
                >
                  <option value="">Select Department</option>
            ...
            {error && (
              <div className="p-4 bg-red-50 text-red-700 rounded-2xl flex items-center gap-3 text-sm border border-red-100">
                <AlertCircle className="w-5 h-5 text-red-500" />
                {error}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-5 bg-indigo-600 text-white font-bold rounded-3xl shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-4 ${
...
const SummaryCard = ({ icon: Icon, label, value, tone }) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200">
      <div className="flex items-center gap-3">
        <div className={`p-2 rounded-xl ${toneMap[tone]}`}>
...
const ToggleCard = ({ active, onClick, icon: Icon, title, subtitle }) => (
  <div
    onClick={onClick}
    className={`cursor-pointer p-5 rounded-2xl border-2 transition-all flex items-center gap-4 ${
      active ? 'border-indigo-600 bg-indigo-50/50' : 'border-slate-100 bg-slate-50 hover:border-slate-200'
    }`}
  >
    <div
      className={`p-3 rounded-xl ${
        active ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-white text-slate-400 shadow-sm'
      }`}
    >
      <Icon className="w-5 h-5" />
    </div>
    <div>
      <p className={`font-bold text-sm ${active ? 'text-indigo-900' : 'text-slate-600'}`}>{title}</p>
      <p className="text-[10px] text-slate-500 uppercase tracking-tight">{subtitle}</p>
    </div>
  </div>
);

const InputWrap = ({ label, icon: Icon, children, disabled = false }) => (
