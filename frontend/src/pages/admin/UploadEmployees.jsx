...
const UploadEmployees = () => {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      setStatus({
        type: 'error',
        message: err.response?.data?.msg || 'Import failed. Please check the file format.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleSingleSubmit = async (e) => {
    ...
    try {
      ...
      setFormStatus({
        type: 'error',
        message: err.response?.data?.msg || 'Failed to create employee',
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fc] px-4 md:px-6 py-4">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-indigo-100">
                Import employee records in bulk or add employees individually.
              </p>
            </div>

            <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-white">
              <p className="text-xs font-semibold text-indigo-100">Import Mode</p>
              <p className="text-xl font-black">{activeTab === 'bulk' ? 'Bulk Upload' : 'Single Entry'}</p>
            </div>
          </div>
        </div>
          ...
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center">
                {activeTab === 'bulk' ? <FileSpreadsheet className="w-5 h-5" /> : <UserPlus className="w-5 h-5" />}
              </div>
              <div>
                <h2 className="text-lg font-bold text-slate-900">
                  {activeTab === 'bulk' ? 'Import Manager' : 'Add Employee'}
                </h2>
                <p className="text-sm text-slate-500">
                  {activeTab === 'bulk' ? 'Upload structured employee sheets' : 'Create one employee record manually'}
                </p>
              </div>
            </div>

            <span className="hidden md:inline-flex rounded-2xl bg-slate-50 border border-slate-200 px-3 py-2 text-xs font-bold uppercase tracking-wider text-slate-600">
              {activeTab === 'bulk' ? 'Excel Support' : 'Single Entry'}
            </span>
          </div>
            ...
            {activeTab === 'bulk' ? (
              <>
                <div className="text-center mb-10">
                  ...
                  <h2 className="text-2xl font-bold text-slate-900">Import Excel File</h2>
                  <p className="text-sm text-slate-500 mt-3 max-w-xl mx-auto leading-relaxed">
                    Upload an Excel file <span className="font-bold text-slate-700">(.xlsx)</span> containing employee records.
                    Make sure your sheet includes all required columns.
                  </p>

                  <div className="flex flex-wrap justify-center gap-2 mt-5 max-w-2xl mx-auto">
                    {['Full Name', 'Email', 'Contact No.', 'Department', 'Unique ID', 'Joining Date', 'Duration'].map((col) => (
                      <span
                        key={col}
                        className="px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-[10px] font-bold border border-slate-200"
                      >
                        {col}
                      </span>
                    ))}
                  ...
                  <div className="mt-8">
                    <button
                      onClick={handleDownloadTemplate}
                      type="button"
                      className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-100 text-xs font-bold"
                    >
                      <CloudDownload className="w-4 h-4" />
                      Download Excel Template Format
                    </button>
                ...
                <form onSubmit={handleImport} className="space-y-8">
                  <div className="w-full">
                    <div className="w-full">
                      <label
                        htmlFor="file-upload"
                        className={`group block w-full cursor-pointer rounded-[2rem] border-2 border-dashed transition-all duration-200 px-6 py-12 text-center ${
                          file
                            ? 'border-indigo-500 bg-indigo-50/30'
                            : 'border-slate-200 bg-slate-50 hover:border-indigo-400 hover:bg-white'
                        }`}
                      >
                        {file ? (
                          <div className="space-y-3">
                            <div className="inline-flex p-3 rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-200">
                          ...
                          <div className="space-y-3">
                            <Import className="mx-auto w-10 h-10 text-slate-300 group-hover:text-indigo-400" />
                            <div>
                              <p className="text-sm font-bold text-slate-700">Choose a file to start</p>
                              <p className="text-[11px] text-slate-400 font-medium uppercase">.xlsx up to 5MB</p>
                            </div>
                          </div>
                        )}
                  ...
                  {status.message && (
                    <div
                      className={`rounded-2xl border px-5 py-4 flex items-center gap-3 text-sm font-medium ${
                        status.type === 'success'
                  ...
                  <div className="w-full">
                    <button
                      type="submit"
                      disabled={loading || !file}
                      className={`w-full max-w-2xl rounded-3xl py-5 px-8 font-bold text-lg flex items-center justify-center gap-3 transition-all ${
                        loading || !file
              ...
              <form onSubmit={handleSingleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Input label="Full Name *" name="fullName" value={employeeData.fullName} onChange={handleInputChange} placeholder="Enter full name" />
                  <Input label="Email" name="email" value={employeeData.email} onChange={handleInputChange} placeholder="Enter email address" />
                  <Input label="Contact No." name="contactNo" value={employeeData.contactNo} onChange={handleInputChange} placeholder="Enter contact number" />
                  ...
                  <div>
                    ...
                    <select
                      ...
                      value={employeeData.domain}
                      onChange={handleInputChange}
                      className="w-full px-4 py-3 rounded-2xl border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                      <option value="">Select Department</option>
                      {departments.map((dept) => (
                        <option key={dept._id || dept.departmentName} value={dept.departmentName}>
                  ...
                  <Input label="Unique ID *" name="uniqueId" value={employeeData.uniqueId} onChange={handleInputChange} placeholder="Enter unique ID" />
                  <Input label="Joining Date" name="appliedDate" type="date" value={employeeData.appliedDate} onChange={handleInputChange} />
                  <Input label="Duration (months)" name="tenureMonths" type="number" value={employeeData.tenureMonths} onChange={handleInputChange} placeholder="e.g. 3" />
                </div>
                ...
                <div className="flex justify-end pt-2">
                  <button
                    type="submit"
                    disabled={submitting}
                    className={`rounded-2xl px-8 py-4 font-bold text-base flex items-center gap-3 transition-all ${
                      submitting
                        ? 'bg-slate-100 text-slate-400 cursor-not-allowed'
                        : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-100'
                    }`}
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 flex items-start gap-3">
          <AlertCircle className="w-4 h-4 text-slate-400 mt-0.5" />
          <p className="text-[11px] text-slate-500 leading-relaxed">
            Note: Dates in Excel must follow the YYYY-MM-DD format. Duplicate Unique IDs are skipped automatically during import.
            Department field accepts department name (not department ID).
          </p>
        </div>
      </div>
    </div>
